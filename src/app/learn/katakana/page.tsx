...
export default function KatakanaPage() {
  ...
  const [selectedChar, setSelectedChar] = useState({ katakana: 'ア', romaji: 'a' });
  ...
  const katakanaList = [
    ...
    { katakana: 'ヤ', romaji: 'ya' }, { katakana: '', romaji: '' },    { katakana: 'ユ', romaji: 'yu' }, { katakana: '', romaji: '' },    { katakana: 'ヨ', romaji: 'yo' },
    { katakana: 'ラ', romaji: 'ra' }, { katakana: 'リ', romaji: 'ri' }, { katakana: 'る', romaji: 'ru' }, { katakana: 'レ', romaji: 're' }, { katakana: 'ロ', romaji: 'ro' },
    { katakana: 'ワ', romaji: 'wa' }, { katakana: '', romaji: '' },    { katakana: '', romaji: '' },    { katakana: '', romaji: '' },    { katakana: 'ヲ', romaji: 'wo' },
    { katakana: 'ン', romaji: 'n' }
  ];

  const quizQuestions = [
    { q: 'Which romanization represents "ツ"?', options: ['tsu', 'shi', 'so', 'n'], answer: 'tsu' },
    { q: 'Which character represents "re"?', options: ['レ', 'ル', 'ラ', 'ロ'], answer: 'レ' },
    { q: 'Which character represents "ma"?', options: ['ム', 'マ', 'モ', 'メ'], answer: 'マ' },
    { q: 'Which romanization represents "キ"?', options: ['ki', 'sa', 'chi', 'ko'], answer: 'ki' },
    { q: 'Which character represents "n" in Katakana?', options: ['ソ', 'ン', 'シ', 'ツ'], answer: 'ン' }
  ];
  ...
  const handleSelectChar = (char: { katakana: string; romaji: string }) => {
    if (!char.katakana) return;
    setSelectedChar(char);
    playSound(char.katakana);
    clearCanvas();
  };
  ...
  const handleQuizAnswer = (option: string) => {
    ...
    if (correct) {
      ...
      setQuizFeedback('✨ Excellent! Correct Katakana matching.');
    } else {
    ...
    setTimeout(() => {
      ...
      if (quizStep < quizQuestions.length - 1) {
        ...
        addHistoryItem('quiz', 'Katakana Sound Matching', 30, Math.round(((quizScore + (correct ? 1 : 0)) / quizQuestions.length) * 100));
      }
    }, 1500);
  };
    ...
    <DashboardLayout>
      <div className="space-y-8 animate-fade-in">
        ...
        <div className="space-y-2">
          <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight">Katakana Loanwords (カタカナ)</h1>
          <p className="text-xs text-neutral-500 max-w-xl font-semibold leading-relaxed">
            The phonetic script for loanwords, scientific names, and special terminology. Learn sounds to read modern menus and signage in Japan.
          </p>
        </div>
                  ...
                  return (
                    <button
                      ...
                      className={`aspect-square rounded-2xl border transition-all flex flex-col items-center justify-center p-2 relative group ${
                        selectedChar.katakana === item.katakana
                    ...
                    >
                      <span className="text-2xl font-extrabold tracking-tight font-serif">{item.katakana}</span>
              ...
              <div className="bg-white dark:bg-[#0c0c0e]/80 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 shadow-sm space-y-6 text-center">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-center gap-2 text-sm font-mono text-neutral-500 font-bold mt-1">
                    ...
                    <span>•</span>
                    <button 
                      onClick={() => playSound(selectedChar.katakana)}
                ...
                <div className="p-4 bg-neutral-50 dark:bg-neutral-900/50 rounded-2xl border border-neutral-150 dark:border-neutral-850 text-left space-y-2 text-xs font-semibold leading-relaxed">
                  ...
                  <p className="text-neutral-600 dark:text-neutral-400">
                    {selectedChar.katakana === 'ア' 
                      ? 'ア looks like an "Axle" or standard key corner.' 
                      : selectedChar.katakana === 'イ'
                      ? 'イ looks like an "Eagle" standing straight.'
                      : `Sound pronunciation represents "${selectedChar.romaji}". Modern clean angular shape lines.`}
                  </p>
                </div>
            ...
            <div className="lg:col-span-5 bg-white dark:bg-[#0c0c0e]/80 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 shadow-sm space-y-6">
              <div className="space-y-2">
                ...
                <p className="text-xs text-neutral-500 leading-relaxed font-semibold">
                  Draw within the gridlines over the light-gray trace layer. Katakana relies on sharp angles and fast pen stops.
                </p>
              </div>

              <div className="grid grid-cols-6 gap-2 max-h-48 overflow-y-auto pr-1">
                {katakanaList.map((item, idx) => {
                  if (!item.katakana) return null;
                  return (
                    <button
                      key={idx}
                      onClick={() => handleSelectChar(item)}
              ...
              <div className="p-4 bg-rose-50 dark:bg-rose-950/20 rounded-2xl border border-rose-200/50 dark:border-rose-900/30 flex items-start gap-2.5 text-xs font-semibold leading-normal">
                ...
                <div className="space-y-0.5">
                  <span className="block text-[10px] font-bold text-rose-800 dark:text-rose-400">Sharp ink angles active</span>
                  <span className="block text-[10px] text-neutral-500 leading-normal">
                    Proper Katakana curves differ severely from Hiragana. Keep lines straight and ends clean.
                  </span>
                </div>
              </div>
            </div>
